...
def main():
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        if recieved_amount < conf["k"]:
            ...
            with open('sflAvg_LSTM_SENTI_size_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
            ...
            with open('sflAvg_LSTM_SENTI_acc_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
                # for row in ret:
