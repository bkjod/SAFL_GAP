...
def main():
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        if recieved_amount < conf["k"]:
            ...
            this_time = time.time()
            with open('aflSGD_LSTM_SENTI_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
                # for row in ret:
                writer.writerow([global_epoch, total_acc, total_loss, this_time - start_time])

            with open('aflSGD_LSTM_SENTI_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
