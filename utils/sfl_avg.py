...
def main():
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        if recieved_amount < conf["k"]:
            ...
            with open('sflAvg_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
            ...
            with open('sflAvg_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
                # for row in ret:
