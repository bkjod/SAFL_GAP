...
def train_model(model, optimizer, data_loader, conf, seq):
    ...
    torch.save(gra_dict, "./sfl/gradient_" + str(seq) + ".pt")
...
def main():
    ...
    for i in range(workers):
        ...
        time.sleep(0.5)
        worker_conf[i] = [resource, get_dataset("../data/", conf["type"], "c", conf, i), 0, 0, "./sfl/global_model_0.pt"]
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        active_client = random.sample(range(100), conf["k"])
        ...
        if recieved_amount < conf["k"]:
            ...
            with open('sflSGD_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
            ...
            with open('sflSGD_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
                # for row in ret:
            ...
            for client_seq_number in range(workers):
                worker_conf[client_seq_number][4] = './sfl/global_model_'+str(global_epoch)+'.pt'
