...
def discord_stale(conf, e, recieved_stamp):
    ...
    with open('aflSGD_'+conf["model_name"]+'_'+conf["type"]+'_staleness_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
...
def train_model(model, optimizer, data_loader, conf, seq):
    ...
    torch.save(model.state_dict(), "./total/local_model" + str(seq) + ".pt")
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        for client_seq_number in range(workers):
            ...
            if time_stamp == 0:
                ...
                local_model.load_state_dict(torch.load("./total/local_model" + str(client_seq_number) + ".pt"))
                total_acc = 0
                total_loss = 0
                for label, loader in eval_loader_list.items():
                    acc, loss = eval_model(local_model, loader) 
                    total_acc += acc
                    total_loss += loss
                total_acc /= len(eval_loader_list)
                total_loss /= len(eval_loader_list)
                ...
                #         cos[name] = c
                #         cos_sim += c
                with open('aflSGD_local_acc_loss.csv', mode='a+', newline='') as file:
                    writer = csv.writer(file)
                    # for row in ret:
                ...
                if  total_acc > (1/conf["CLASS_NUM"])*110:
                    ...
                    with open('aflSGD_using_local_acc_loss.csv', mode='a+', newline='') as file:
                        writer = csv.writer(file)
                        # for row in ret:
                        writer.writerow([client_seq_number, worker_conf[client_seq_number][3], global_epoch, total_acc, total_loss, cos_sim, cos])
                ...
                worker_conf[client_seq_number][4] = "./total/local_model" + str(client_seq_number) + ".pt"
                uploaded_model += 1
            else:
        ...
        if recieved_amount < conf["k"]:
            ...
            this_time = time.time()
            with open('aflSGD_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
                # for row in ret:
                writer.writerow([global_epoch, total_acc, total_loss, this_time - start_time])

            with open('aflSGD_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
