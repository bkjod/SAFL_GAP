...
def discord_stale(conf, e, recieved_stamp):
    ...
    with open('aflAT_'+conf["model_name"]+'_'+conf["type"]+'_staleness_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([e, decay_stamp, sum(decay_stamp)])


# aggregrate the model
def aggregrate_model(global_model, recieved_model, uploading_models, conf, e):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
def train_model(model, optimizer, data_loader, conf, seq):
    ...
    torch.save(model.state_dict(), "./KAFL/gradient_" + str(seq) + ".pt")
    torch.save(model.state_dict(), "./KAFL/local_model" + str(seq) + ".pt")
...
def main():
    ...
    for i in range(workers):
        ...
        worker_conf[i] = [resource, loader, 0, 0, "./KAFL/global_model_0.pt",len(loader)]
    ...
    if conf["model_name"] == "resnet18":
        ...
        pass
    torch.save(global_model.state_dict(), "./KAFL/global_model_0.pt")
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        for client_seq_number in range(workers):
            ...
            if time_stamp == 0:
                ...
                gra =  "./KAFL/gradient_" + str(client_seq_number) + ".pt"
                local_model = resnet_model.ResNet18(num=conf["CLASS_NUM"]).to(device)

                local_model.load_state_dict(torch.load("./KAFL/local_model" + str(client_seq_number) + ".pt"))
                ...
                #         cos[name] = c
                #         cos_sim += c
                with open('aflAT_local_acc_loss.csv', mode='a+', newline='') as file:
                    writer = csv.writer(file)
                    # for row in ret:
                ...
                #     with open('aflAT_using_local_acc_loss.csv', mode='a+', newline='') as file:
                #         writer = csv.writer(file)
                #         # for row in ret:
                #         writer.writerow([client_seq_number, worker_conf[client_seq_number][3], global_epoch, total_acc, total_loss, cos_sim, cos])

                have_recieved_model.append([client_seq_number, gra, worker_conf[client_seq_number][5]])          # update the model to server
                have_recieved_stamp.append(worker_conf[client_seq_number][3])
                worker_conf[client_seq_number][2] = 0                          # reset the time stamp of this client
                worker_conf[client_seq_number][4] = "./KAFL/local_model" + str(client_seq_number) + ".pt"
                uploaded_model += 1
        ...
        if recieved_amount < conf["k"]:
            ...
            global_model = aggregrate_model(global_model, have_recieved_model, uploading_models, conf, global_epoch) 
            ...
            torch.save(global_model.state_dict(), "./KAFL/global_model_"+str(global_epoch)+".pt")

            # notice the newest global model to each client
            for client_seq_number in range(workers):
                worker_conf[client_seq_number][4] = './KAFL/global_model_'+str(global_epoch)+'.pt'

            print("Finish aggregrate and leave ", len(have_recieved_model), " models!")

        
            this_time = time.time()
            with open('aflAT_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
                # for row in ret:
                writer.writerow([global_epoch, total_acc, total_loss, this_time - start_time])

            with open('aflAT_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
